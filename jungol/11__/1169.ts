...
function m1(dice_idx: number) {
  if (dice_idx == n) {
    nodeIO.write(vector.map((i) => i + 1).join(" "));
    return;
  }
  for (let i = 0; i < 6; i++) {
    vector.push(i);
...
function m2(dice_idx: number) {
  ...
  if (dice_idx == 0) {
    ...
    let lastOne: number = vector.at(-1)!;
    for (let i = lastOne; i < 6; i++) {
      vector.push(i);
      m2(dice_idx + 1);
      vector.pop();
    }
