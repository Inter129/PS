...
// Init
int arr[canvas_size + margin_size][canvas_size + margin_size];

// Func
void input(){
    int A, B;
    cin >> A >> B;
    for(int i=A+1; i<=A+box_size; i++){
        for(int j=B+1; j<=B+box_size; j++){
            arr[i][j] = 1;
        }
    }
}

bool is_only_one_block(int a, int b){
...
int get_box_radius(){
    ...
    for(int i=1; i<=canvas_size; i++){
        for(int j=1; j<=canvas_size; j++){
            if(is_only_one_block(arr[i][j], arr[i+1][j])) ans++;
            if(is_only_one_block(arr[i][j], arr[i][j+1])) ans++;
        }
    }
    return ans;
}
